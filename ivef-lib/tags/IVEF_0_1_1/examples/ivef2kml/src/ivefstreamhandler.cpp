...
void IVEFStreamHandler::connectToServer(QString host, int port, QString user, QString password, QString logFileName, bool slipstream, bool statistics) {
    ...
    std::cout << QString("Opening connection to %1:%2").arg(host, portString).toLatin1().data() << std::endl;
    ...
    if ( m_slipstream ) {
        ...
        if (inflateInit(&m_strm) != Z_OK) {
            std::cout << "Error initiating zlib stream" << std::endl;
        }
    }
#endif
    ...
    if (logFileName != "") {
        ...
        if (!file->open(QIODevice::WriteOnly | QIODevice::Text)) {
            std::cerr << QString("Error opening file: %1 for writing").arg(logFileName).toLatin1().data() << std::endl;
            std::exit(1);
        }
...
void IVEFStreamHandler::slotConnected() {
    std::cout << QString("Connection established logging in as %1").arg( m_user ).toLatin1().data() << std::endl;
...
void IVEFStreamHandler::slotStatisticsTimerFired() {
    ...
    std::cout << "In = " << bitsPerSecIn << " bit/sec (" << kBitsPerSecIn << " kbits/sec) out = " << bitsPerSecOut << " bit/sec (" << kBitsPerSecOut << " kbits/sec)" << std::endl;
    m_bytesIn = 0;
    m_bytesOut = 0;
}
